    ...
    def parse_tagged_sents(
        ...
        try:
            ...
            return (
                ...
                for item in open(output_path, encoding="utf8").read().split("\n\n")  # noqa: PTH123
                if item.strip()
            )

        finally:
            if input_path and os.path.exists(input_path):  # noqa: PTH110
                os.remove(input_path)  # noqa: PTH107
            if output_path and os.path.exists(output_path):  # noqa: PTH110
...
class TurboParser(ParserI):
    ...
    def tagged_parse_sents(
        ...
        try:
            ...
            self.interface.parse(input_path, output_path)
...
class SpacyDependencyParser(MaltParser):
    """A Dependency Parser based on Spacy."""
    ...
    def _setup_model(self: "SpacyDependencyParser") -> None:
        ...
        try:
            ...
            msg = "Something wrong loading the dependencyParser . Checkout for correctness or existence of path"
            raise ValueError(msg) from e

    def _add_sentence2dict(self: "SpacyDependencyParser", sent: list[str]) -> None:
        ...
        """
        self.peykare_dict[" ".join(list(sent))] = list(sent)

    def _custom_tokenizer(self: "SpacyDependencyParser", text: str) -> Doc:
