...
class SpacyPOSTagger(POSTagger):
    ...
    def __init__(
        ...
        # Usually an output directory for spacy model contain two other directory name model-last , model-best
        # You should give model_path like this : output/model-last
    ...
    def _setup(self: "SpacyPOSTagger"):
        ...
        This function initializes and configures the spaCy model, checks for GPU availability,
        and prepares the training and testing datasets in spaCy format.
    ...
    def _setup_model(self: "SpacyPOSTagger", sents):
        ...
        Args:
            sents: List of sentences to set up the dictionary.
    ...
    def _set_peykare_dictionary(self: "SpacyPOSTagger", sents):
        ...
        """
        self.peykare_dict = {
            " ".join(list(item)): list(item) for item in sents
        }
    ...
    def tag(self: "SpacyPOSTagger", tokens, universal_tag=True):
        ...
        Args:
            tokens (list[str]): لیستی از توکن‌های یک جمله که باید برچسب‌گذاری شود.
            universal_tag: Universal tagging.
        ...
        doc = self.tagger([" ".join(list(tokens))])
    ...
    def tag_sents(
        ...
        docs = list(
            self.tagger.pipe(
                (" ".join(list(sent)) for sent in sents),
        ...
        return [
            list(zip(tok, tag, strict=True))
            for tok, tag in zip(sents, tags, strict=True)
        ]
    ...
    def train(
        ...
        Args:
            ...
            use_direct_config: Use direct config file.
    ...
    def evaluate(self: "SpacyPOSTagger", test_sents, batch_size):
        ...
        Args:
            test_sents: List of sentences for evaluation.
        ...
        if self.tagger:
            ...
            msg = "Model does not exist.Please train a new one with train method of this instance or give a model_path , setup the model with self._setup_model() and then call evaluate"
            raise ValueError(msg)
    ...
    def _evaluate_tags(
        ...
        predictions_cleaned = []
        golds_cleaned = []
        get_tag_func = self._get_ez_tags if use_ez_tags else self._remove_ez_tags
        ...
        print(
            f"Precision: {precision_score(golds_cleaned, predictions_cleaned, average='weighted'):.5f}",
        )
        print(
            f"Recall: {recall_score(golds_cleaned, predictions_cleaned, average='weighted'):.5f}",
        )
        print(
            f"F1-Score: {f1_score(golds_cleaned, predictions_cleaned, average='macro'):.5f}",
        )
    ...
    def _get_labels_and_predictions(
        ...
        prediction_labels = self.tag_sents(tokens, batch_size=batch_size)
